...
console.log("form====>",form);
  ...
  const totalRequired = useMemo(() => REQUIRED_KEYS.length, []);
  const filledRequired = totalRequired - missingFields.length;
  const progressPct = Math.max(0, Math.min(100, Math.round((filledRequired / Math.max(1, totalRequired)) * 100)));
  ...
  // Simple validators and normalizers
  const isValidEmail = (s: string) => /.+@.+\..+/.test(String(s).trim());
  const isValidPhone = (s: string) => String(s).replace(/[^0-9]/g, '').length >= 7;
  ...
  // Track focused field for subtle elevation
  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={[styles.headerWrap, { paddingTop: insets.top + 10 }]}> 
        {/* Decorative bubbles */}
        <View style={styles.headerBubbleA} />
        <View style={styles.headerBubbleB} />
        <View style={styles.headerTopRow}>
          ...
          <View style={styles.headerBreadcrumbs}>
            <Text style={styles.headerCrumbText}>Leads</Text>
            <Ionicons name="chevron-forward" size={12} color="#9CA3AF" style={{ marginHorizontal: 2 }} />
            <Text style={styles.headerCrumbActive}>New</Text>
          </View>
        </View>
        ...
        <View style={styles.headerProgressRow}>
          <View style={styles.headerProgressBarBg}>
            <View style={[styles.headerProgressBarFg, { width: `${progressPct}%` }]} />
          </View>
          <Text style={styles.headerProgressText}>{filledRequired}/{totalRequired} fields</Text>
        </View>
        {/* Quick chips */}
        <View style={styles.headerChipsRow}>
          {form.location ? (
            <View style={[styles.headerChip, { backgroundColor: '#dbeafe' }]}>
              <Ionicons name="locate-outline" size={12} color="#1d4ed8" />
              <Text style={[styles.headerChipText, { color: '#1d4ed8' }]}>Location On</Text>
            </View>
          ) : (
            <View style={[styles.headerChip, { backgroundColor: '#fee2e2' }]}>
              <Ionicons name="warning-outline" size={12} color="#b91c1c" />
              <Text style={[styles.headerChipText, { color: '#b91c1c' }]}>Location Off</Text>
            </View>
          )}
          <View style={[styles.headerChip, { backgroundColor: '#dcfce7' }]}>
            <Ionicons name="shield-checkmark-outline" size={12} color="#166534" />
            <Text style={[styles.headerChipText, { color: '#166534' }]}>Autosave Safe</Text>
          </View>
        </View>
      </View>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView style={styles.scroll} contentContainerStyle={{ paddingBottom: 120 }} keyboardShouldPersistTaps="handled">
          ...
          <Card styleProp={focusedKey && ['lead_name','lead_owner','gender','source','status','lead_type','associate','request_type','location','service_type'].includes(focusedKey) ? styles.cardFocused : undefined}>
            <CardTitle>Lead Details</CardTitle>
            <DateField label="Date" value={form.date} onChange={(v) => setField('date')(v)} required />
            <Divider />
            <Field label="Lead Name" value={form.lead_name} onChangeText={setField('lead_name')} required leftIcon="person-outline" onFocus={() => setFocusedKey('lead_name')} />
            <Divider />
          ...
          {/* Contact Information */}
          <Card styleProp={focusedKey && ['email_id','mobile_no','phone','website'].includes(focusedKey) ? styles.cardFocused : undefined}>
            <CardTitle>Contact Information</CardTitle>
            <Field label="Email" value={form.email_id} onChangeText={setField('email_id')} keyboardType="email-address" required leftIcon="mail-outline"
              validate={(v)=>({ valid: isValidEmail(v), msg: isValidEmail(v)?'':'Invalid email'})}
              onFocus={() => setFocusedKey('email_id')}
            />
            <Divider />
            <PhoneField label="Mobile Number" value={form.mobile_no} onChangeText={setField('mobile_no')} required onFocus={() => setFocusedKey('mobile_no')} />
            <Divider />
            <PhoneField label="Phone Number" value={form.phone} onChangeText={setField('phone')} required onFocus={() => setFocusedKey('phone')} />
            <Divider />
            <Field label="Website" value={form.website} onChangeText={setField('website')} autoCapitalize="none" required leftIcon="globe-outline"
              onBlur={() => setField('website')(normalizeUrl(form.website))}
              validate={(v)=>({ valid: isValidUrl(v), msg: isValidUrl(v)?'':'Invalid URL'})}
      ...
      <View style={[styles.bottomBar, { paddingBottom: Math.max(insets.bottom, 10) }]}>
        <View style={styles.bottomProgressWrap}>
          ...
          <Text style={styles.headerProgressText}>{filledRequired}/{totalRequired}</Text>
        </View>
        <View style={styles.bottomActions}>
          ...
          <Pressable style={[styles.bottomBtnPrimary, !canSave && styles.bottomBtnDisabled]} disabled={!canSave} onPress={onSave} accessibilityRole="button">
            {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.bottomBtnPrimaryText}>Save</Text>}
          </Pressable>
        </View>
      </View>
    </View>
...
/* UI Helpers */
function Card({ children, styleProp }: { children: React.ReactNode; styleProp?: any }) {
  return <View style={[styles.card, styleProp]}>{children}</View>;
}
...
function Field({ label, value, onChangeText, keyboardType, autoCapitalize, required, placeholder, leftIcon, validate, onFocus, onBlur }: { label: string; value: string; onChangeText: (t: string) => void; keyboardType?: any; autoCapitalize?: 'none' | 'sentences' | 'words' | 'characters'; required?: boolean; placeholder?: string; leftIcon?: string; validate?: (v: string) => { valid: boolean; msg?: string }; onFocus?: () => void; onBlur?: () => void }) {
  const [touched, setTouched] = useState(false);
  const res = validate ? validate(value) : { valid: true };
  return (
    <View style={{ paddingVertical: 10 }}>
      ...
      <View style={styles.inputWrap}>
        {leftIcon ? <Ionicons name={leftIcon as any} size={16} color="#6B7280" style={{ marginHorizontal: 8 }} /> : null}
        <TextInput
          value={value}
          onChangeText={onChangeText}
          placeholder={placeholder || label}
          placeholderTextColor="#9CA3AF"
          style={[styles.input, { flex: 1, borderWidth: 0, paddingVertical: 8, paddingHorizontal: 0 }]}
          keyboardType={keyboardType}
          autoCapitalize={autoCapitalize}
          onFocus={() => { setTouched(true); onFocus?.(); }}
          onBlur={() => { onBlur?.(); }}
        />
        {touched && validate ? (
          res.valid ? <Ionicons name="checkmark-circle" size={16} color="#10b981" style={{ marginRight: 8 }} /> : <Ionicons name="alert-circle" size={16} color="#ef4444" style={{ marginRight: 8 }} />
        ) : null}
      </View>
      {touched && validate && !res.valid && res.msg ? (
        <Text style={styles.inputHelperError}>{res.msg}</Text>
      ) : null}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  headerBubbleA: { position: 'absolute', right: -30, top: -20, width: 120, height: 120, borderRadius: 60, backgroundColor: 'rgba(255,255,255,0.06)' },
  headerBubbleB: { position: 'absolute', left: -40, top: 24, width: 90, height: 90, borderRadius: 45, backgroundColor: 'rgba(255,255,255,0.04)' },
  headerTopRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  headerLeft: { flexDirection: 'row', alignItems: 'center' },
  headerTitle: { color: '#ffffff', fontWeight: '700', fontSize: 16, marginLeft: 2 },
  headerBreadcrumbs: { flexDirection: 'row', alignItems: 'center' },
  headerCrumbText: { color: '#9CA3AF', fontSize: 12, fontWeight: '600' },
  headerCrumbActive: { color: '#E5E7EB', fontSize: 12, fontWeight: '700' },
  ...
  headerProgressRow: { marginTop: 10, paddingRight: 12, paddingLeft: 12, flexDirection: 'row', alignItems: 'center', gap: 8 },
  headerProgressBarBg: { flex: 1, height: 6, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 4 },
  headerProgressBarFg: { height: 6, backgroundColor: '#60A5FA', borderRadius: 4 },
  headerProgressText: { color: '#D1D5DB', fontSize: 12, fontWeight: '600' },
  headerChipsRow: { flexDirection: 'row', gap: 8, paddingHorizontal: 12, marginTop: 8 },
  headerChip: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingVertical: 4, paddingHorizontal: 8, borderRadius: 999 },
  ...
  cardFocused: { elevation: 3, shadowColor: '#000', shadowOpacity: 0.12, shadowRadius: 6, shadowOffset: { width: 0, height: 3 }, borderColor: '#D1D5DB' },
  ...
  inputWrap: { borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 8, backgroundColor: '#FFFFFF', flexDirection: 'row', alignItems: 'center' },
  inputHelperError: { color: '#ef4444', fontSize: 11, marginTop: 4 },
  ...
  bottomBar: { position: 'absolute', left: 0, right: 0, bottom: 0, backgroundColor: '#ffffff', borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: '#E5E7EB', paddingTop: 8, paddingHorizontal: 12 },
  bottomProgressWrap: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 },
  bottomActions: { flexDirection: 'row', gap: 10 },
  bottomBtnPrimary: { flex: 1, backgroundColor: '#2563EB', paddingVertical: 12, borderRadius: 10, alignItems: 'center' },
  bottomBtnDisabled: { backgroundColor: '#93C5FD' },
