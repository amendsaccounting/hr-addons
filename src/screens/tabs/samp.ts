...
export default function LeaveScreen() {
  ...
  const submitLeaveRequest = React.useCallback(async () => {
    ...
    try {
      ...
      const conflict = requests.some(r => {
        const status = (r.status || '').toLowerCase();
        if (status === 'rejected') return false; // allow overlapping with rejected requests
        const rFrom = new Date(r.from);
        const rTo = new Date(r.to);
        return !(newTo < rFrom || newFrom > rTo); // overlap check
      });
      if (conflict) {
Alert.alert(
  "Leave Already Applied",
  "You’ve already applied for leave on the selected date. Please choose another date."
);
        return;
      }
    } catch {}
